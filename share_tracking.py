...
def process_data(portfolio):
    ...
    portfolio.loc[:, idx[['Shares', 'Price', 'Div'], :]] = (
                                    portfolio.loc[:, idx[['Shares', 'Price', 'Div'], :]].fillna(0))
    #merged_portfolio[['Shares', 'Price', 'Div']] = merged_portfolio[['Shares', 'Price', 'Div']].fillna(0)
    
    
    
    # Accumulated shares  
    Accum = (portfolio['Shares'][~np.isnan(portfolio['Shares'])].cumsum()
                                     .ffill()
                                     .fillna(0))
    ...
    Buy_amt = ((portfolio['Price'] * portfolio['Shares'])
               .fillna(0))    
    
    if 'Adjustments' in portfolio.columns:
        # Cash flow adjustments due to demerger/acquisition event.
        cols_with_adjustments = portfolio['Adjustments'].columns
    
        portfolio.loc[:, idx['Adjustments', :]] = portfolio.loc[:, idx['Adjustments', :]].fillna(0)
        ...
        #merged_portfolio.loc[:, idx[['Div'], idx[cols_with_adjustments]]] = (
        #    merged_portfolio.loc[:, idx[['Div'], idx[cols_with_adjustments]]]
        #    + merged_portfolio.loc[:, idx[['Adjustments'], idx[cols_with_adjustments]]]
        #    * merged_portfolio.loc[:, idx[['Div'], idx[cols_with_adjustments]]].cumsum()) 
        
        Div = (portfolio['Div'][cols_with_adjustments] +
               portfolio['Adjustments'][cols_with_adjustments] * 
               portfolio['Div'][cols_with_adjustments].cumsum())
        
        Div.columns = pd.MultiIndex.from_product([['Div'], Div.columns],
                                                 names = ['Params', 'Company'])
    else:
        # Reset column names
    ...
    #Div_tot = (merged_portfolio['Div'] * merged_portfolio.loc[:, idx['Accum', :]]).round(2)
    Div_tot = (portfolio['Div'] * Accum).round(2)
    # Reset column names
    Div_tot.columns = pd.MultiIndex.from_product([['Div_tot'], 
                                                  Div_tot.columns.get_level_values(1)],
                                                  names = ['Params', 'Company'])
...
def stock_summary(portfolio, index, date=None, styles=True, calc_method="basic"):
    ...
    df["Daily Return (%)"] = (
        calc.daily_pct_gain(price.drop(labels=index, axis=1)).iloc[-1] * 100
    )
