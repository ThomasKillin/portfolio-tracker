    ...
    if 'portfolio' in st.session_state:     
        ...
        with st.sidebar:
            start_date = st.date_input(':date: Select start date', 
                                        ...
                                        key=f"start_date_input_{int(time.time() * 1000)}")
...
def display_data():
    ...
    if 'portfolio' in st.session_state:     
        ...
        #st.session_state['display'] = True
        with st.sidebar:
            unique_key = get_unique_key("start_date_input")
            start_date = st.date_input(':date: Select start date', 
                                        min_value = st.session_state['portfolio'].index[0],
                                        max_value = st.session_state['portfolio'].index[-2],
                                        value = st.session_state['portfolio'].index[0],
                                        key=unique_key)  # Added unique key
            st.session_state['start_date'] = start_date
...
with st.sidebar:
    ...
    if file is not None:
        button_key = get_unique_key("button")
        button = st.button('Get share price data', type="primary", key=button_key)  # Added unique key
